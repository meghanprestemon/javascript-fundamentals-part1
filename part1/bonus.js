...
function caesarShiftChar(characters) {
    var char = characters.toLowerCase();
    var currCharUni = char.codePointAt(0);
    var currCharShift = 0;

    if ((currCharUni < 97) || (currCharUni > 122)) {
      return char;
    } else if ((currCharUni + 13) > 122) {
      currCharShift = currCharUni + 13 - 26;
    } else {
      currCharShift = currCharUni + 13;
    }

    var newChar = String.fromCodePoint(currCharShift);
...
function encodeMessage(message) {
  var char = message.toLowerCase();
  var shiftedString = "";

  for (var i = 0; i < char.length; i++) {
    var currChar = char[i];
    var currCharUni = char.codePointAt(i);
    var currCharShift = 0;

    if ((currCharUni < 97) || (currCharUni > 122)) {
      shiftedString += currChar;
      continue;
    } else if ((currCharUni + 13) > 122) {
      currCharShift = currCharUni + 13 - 26;
    } else {
